import sys
import tkinter as tk # Keep for messagebox/filedialog as fallback if needed, but prefer Qt's
from tkinter import messagebox # Keep for simple messages initially
...
import copy
import json
from collections import defaultdict # 用于分组 MPO 端口
...
from PySide6.QtWidgets import (
    ...
    QHeaderView, QListWidget, QSplitter # <--- 添加 QSplitter 导入
)
...
# --- 数据结构 (V20 状态重构) ---
class Device:
    """代表一个 MediorNet 设备 - V20 状态重构"""
    def __init__(self, id, name, type, mpo_ports=0, lc_ports=0, sfp_ports=0):
        self.id = id
        self.name = name
        self.type = type # 'MicroN UHD', 'HorizoN', 'MicroN'
        self.mpo_total = mpo_ports
        self.lc_total = lc_ports
        self.sfp_total = sfp_ports # 仅用于 MicroN
        self.reset_ports()

    def reset_ports(self):
        """重置连接计数和已用端口记录"""
        self.connections = 0.0 # 使用浮点数以精确处理 MPO 子通道
        self.port_connections = {} # Single source of truth for used ports {port_name: target_device_name}
    ...
    def get_all_available_ports(self):
        """获取所有当前可用端口的列表 (基于 port_connections)"""
    ...
    def return_port(self, port_name):
        ...
        if port_name in self.port_connections:
            ...
            print(f"警告: 尝试归还未记录的端口 {port_name}")

    def get_available_port(self, port_type, target_device_name):
        """(计算/填充用) 获取一个指定类型的可用端口并标记为已用"""
        possible_ports = []
        if port_type == 'LC':
            possible_ports = [f"LC{i+1}" for i in range(self.lc_total)]
        elif port_type == 'MPO':
            for i in range(self.mpo_total):
                base = f"MPO{i+1}"
                possible_ports.extend([f"{base}-Ch{j+1}" for j in range(4)])
            random.shuffle(possible_ports) # Mesh 计算时打乱 MPO 顺序
        elif port_type == 'SFP':
            possible_ports = [f"SFP{i+1}" for i in range(self.sfp_total)]
    ...
    def get_specific_available_port(self, port_type_prefix):
        """(环形计算/填充用) 获取一个指定前缀的可用端口 (不打乱顺序)"""
        possible_ports = []
        if port_type_prefix == 'LC':
            possible_ports = sorted([f"LC{i+1}" for i in range(self.lc_total)], key=lambda x: int(x[2:]))
        elif port_type_prefix == 'MPO':
            for i in range(self.mpo_total):
                base = f"MPO{i+1}"
                possible_ports.extend(sorted([f"{base}-Ch{j+1}" for j in range(4)], key=lambda x: int(x.split('-Ch')[-1])))
        elif port_type_prefix == 'SFP':
            possible_ports = sorted([f"SFP{i+1}" for i in range(self.sfp_total)], key=lambda x: int(x[3:]))

        used_ports = set(self.port_connections.keys())
        for port in possible_ports:
            if port not in used_ports:
                return port # 只返回找到的第一个可用端口名，不占用
        return None
...
def calculate_mesh_connections(devices):
    """计算 Mesh 连接 - 改进均衡性 (使用 V20 Device)"""
    if len(devices) < 2: return [], {}
    connections = []
    temp_devices = [copy.deepcopy(dev) for dev in devices]
    for d in temp_devices: d.reset_ports()
    device_map = {dev.id: dev for dev in temp_devices}
    possible_pairs_ids = list(itertools.combinations([d.id for d in temp_devices], 2))
    connection_made_in_full_pass = True
    while connection_made_in_full_pass:
        connection_made_in_full_pass = False
        random.shuffle(possible_pairs_ids)
        for dev1_id, dev2_id in possible_pairs_ids:
            dev1, dev2 = device_map[dev1_id], device_map[dev2_id]
            made_connection_this_pair = False
            # (规则检查和连接添加逻辑保持不变, 使用副本的 get_available_port)
            if dev1.type in ['MicroN UHD', 'HorizoN'] and dev2.type in ['MicroN UHD', 'HorizoN']: # LC-LC
                port1 = dev1.get_available_port('LC', dev2.name); port2 = dev2.get_available_port('LC', dev1.name) if port1 else None
                if port1 and port2: connections.append((next(d for d in devices if d.id == dev1_id), port1, next(d for d in devices if d.id == dev2_id), port2, 'LC-LC (100G)')); made_connection_this_pair = True
                elif port1: dev1.return_port(port1)
            if not made_connection_this_pair and dev1.type in ['MicroN UHD', 'HorizoN'] and dev2.type in ['MicroN UHD', 'HorizoN']: # MPO-MPO
                port1 = dev1.get_available_port('MPO', dev2.name); port2 = dev2.get_available_port('MPO', dev1.name) if port1 else None
                if port1 and port2: connections.append((next(d for d in devices if d.id == dev1_id), port1, next(d for d in devices if d.id == dev2_id), port2, 'MPO-MPO (25G)')); made_connection_this_pair = True
                elif port1: dev1.return_port(port1)
            if not made_connection_this_pair and dev1.type == 'MicroN' and dev2.type == 'MicroN': # SFP-SFP
                port1 = dev1.get_available_port('SFP', dev2.name); port2 = dev2.get_available_port('SFP', dev1.name) if port1 else None
                if port1 and port2: connections.append((next(d for d in devices if d.id == dev1_id), port1, next(d for d in devices if d.id == dev2_id), port2, 'SFP-SFP (10G)')); made_connection_this_pair = True
                elif port1: dev1.return_port(port1)
            if not made_connection_this_pair: # MPO-SFP
                uhd_dev, micron_dev, original_uhd, original_micron = (None, None, None, None)
                if dev1.type in ['MicroN UHD', 'HorizoN'] and dev2.type == 'MicroN': uhd_dev, micron_dev, original_uhd, original_micron = dev1, dev2, next(d for d in devices if d.id == dev1_id), next(d for d in devices if d.id == dev2_id)
                elif dev2.type in ['MicroN UHD', 'HorizoN'] and dev1.type == 'MicroN': uhd_dev, micron_dev, original_uhd, original_micron = dev2, dev1, next(d for d in devices if d.id == dev2_id), next(d for d in devices if d.id == dev1_id)
                if uhd_dev and micron_dev:
                    port_uhd = uhd_dev.get_available_port('MPO', micron_dev.name)
                    if port_uhd:
                        port_micron = micron_dev.get_available_port('SFP', uhd_dev.name)
                        if port_micron: connections.append((original_uhd, port_uhd, original_micron, port_micron, 'MPO-SFP (10G)')); made_connection_this_pair = True
                        else: uhd_dev.return_port(port_uhd)
            if made_connection_this_pair: connection_made_in_full_pass = True
...
def _find_best_single_link(dev1_copy, dev2_copy):
    ...
    if dev1_copy.type in ['MicroN UHD', 'HorizoN'] and dev2_copy.type in ['MicroN UHD', 'HorizoN']:
        port1 = dev1_copy.get_specific_available_port('LC'); port2 = dev2_copy.get_specific_available_port('LC') if port1 else None
        if port1 and port2: dev1_copy.use_specific_port(port1, dev2_copy.name); dev2_copy.use_specific_port(port2, dev1_copy.name); return port1, port2, 'LC-LC (100G)'
    if dev1_copy.type in ['MicroN UHD', 'HorizoN'] and dev2_copy.type in ['MicroN UHD', 'HorizoN']:
        port1 = dev1_copy.get_specific_available_port('MPO'); port2 = dev2_copy.get_specific_available_port('MPO') if port1 else None
        if port1 and port2: dev1_copy.use_specific_port(port1, dev2_copy.name); dev2_copy.use_specific_port(port2, dev1_copy.name); return port1, port2, 'MPO-MPO (25G)'
    if dev1_copy.type == 'MicroN' and dev2_copy.type == 'MicroN':
        port1 = dev1_copy.get_specific_available_port('SFP'); port2 = dev2_copy.get_specific_available_port('SFP') if port1 else None
        if port1 and port2: dev1_copy.use_specific_port(port1, dev2_copy.name); dev2_copy.use_specific_port(port2, dev1_copy.name); return port1, port2, 'SFP-SFP (10G)'
    ...
    if uhd_dev and micron_dev:
        port_uhd = uhd_dev.get_specific_available_port('MPO'); port_micron = micron_dev.get_specific_available_port('SFP') if port_uhd else None
        if port_uhd and port_micron:
            uhd_dev.use_specific_port(port_uhd, micron_dev.name); micron_dev.use_specific_port(port_micron, uhd_dev.name)
            if dev1_copy == uhd_dev: return port_uhd, port_micron, 'MPO-SFP (10G)'
...
def calculate_ring_connections(devices):
    ...
    for d in temp_devices: d.reset_ports()
    device_map = {dev.id: dev for dev in temp_devices}
    sorted_dev_ids = sorted([d.id for d in devices])
    ...
    for i in range(num_devices):
        ...
        if port1 and port2:
            connections.append((original_dev1, port1, original_dev2, port2, conn_type))
            link_established[i] = True
            # print(f"环形连接: {original_dev1.name}[{port1}] <-> {original_dev2.name}[{port2}]") # 减少打印
...
# --- Mesh 填充函数 (保持不变) ---
def fill_mesh_connections(devices):
    """使用剩余端口，以 Mesh 方式填充连接"""
    if len(devices) < 2: return []
    new_connections = []
    temp_devices = [copy.deepcopy(dev) for dev in devices] # 使用当前状态的副本
    device_map = {dev.id: dev for dev in temp_devices}
    possible_pairs_ids = list(itertools.combinations([d.id for d in temp_devices], 2))
    # print(f"DIAG (Fill): 开始填充，共有 {len(possible_pairs_ids)} 对设备。") # 减少打印
    ...
    while connection_made_in_full_pass:
        ...
        random.shuffle(possible_pairs_ids)
        for dev1_id, dev2_id in possible_pairs_ids:
            dev1_copy, dev2_copy = device_map[dev1_id], device_map[dev2_id]
            made_connection_this_pair = False
            # (规则检查和连接添加逻辑保持不变, 使用副本的 get_available_port)
            if dev1_copy.type in ['MicroN UHD', 'HorizoN'] and dev2_copy.type in ['MicroN UHD', 'HorizoN']: # LC-LC
                port1 = dev1_copy.get_available_port('LC', dev2_copy.name); port2 = dev2_copy.get_available_port('LC', dev1_copy.name) if port1 else None
                if port1 and port2: original_dev1 = next(d for d in devices if d.id == dev1_id); original_dev2 = next(d for d in devices if d.id == dev2_id); new_connections.append((original_dev1, port1, original_dev2, port2, 'LC-LC (100G)')); made_connection_this_pair = True
                elif port1: dev1_copy.return_port(port1)
            if not made_connection_this_pair and dev1_copy.type in ['MicroN UHD', 'HorizoN'] and dev2_copy.type in ['MicroN UHD', 'HorizoN']: # MPO-MPO
                port1 = dev1_copy.get_available_port('MPO', dev2_copy.name); port2 = dev2_copy.get_available_port('MPO', dev1_copy.name) if port1 else None
                if port1 and port2: original_dev1 = next(d for d in devices if d.id == dev1_id); original_dev2 = next(d for d in devices if d.id == dev2_id); new_connections.append((original_dev1, port1, original_dev2, port2, 'MPO-MPO (25G)')); made_connection_this_pair = True
                elif port1: dev1_copy.return_port(port1)
            if not made_connection_this_pair and dev1_copy.type == 'MicroN' and dev2_copy.type == 'MicroN': # SFP-SFP
                port1 = dev1_copy.get_available_port('SFP', dev2_copy.name); port2 = dev2_copy.get_available_port('SFP', dev1_copy.name) if port1 else None
                if port1 and port2: original_dev1 = next(d for d in devices if d.id == dev1_id); original_dev2 = next(d for d in devices if d.id == dev2_id); new_connections.append((original_dev1, port1, original_dev2, port2, 'SFP-SFP (10G)')); made_connection_this_pair = True
                elif port1: dev1_copy.return_port(port1)
            if not made_connection_this_pair: # MPO-SFP
                uhd_dev, micron_dev, original_uhd, original_micron = (None, None, None, None)
                if dev1_copy.type in ['MicroN UHD', 'HorizoN'] and dev2_copy.type == 'MicroN': uhd_dev, micron_dev, original_uhd, original_micron = dev1_copy, dev2_copy, next(d for d in devices if d.id == dev1_id), next(d for d in devices if d.id == dev2_id)
                elif dev2_copy.type in ['MicroN UHD', 'HorizoN'] and dev1_copy.type == 'MicroN': uhd_dev, micron_dev, original_uhd, original_micron = dev2_copy, dev1_copy, next(d for d in devices if d.id == dev2_id), next(d for d in devices if d.id == dev1_id)
                if uhd_dev and micron_dev:
                    port_uhd = uhd_dev.get_available_port('MPO', micron_dev.name)
                    if port_uhd:
                        port_micron = micron_dev.get_available_port('SFP', uhd_dev.name)
                        if port_micron: new_connections.append((original_uhd, port_uhd, original_micron, port_micron, 'MPO-SFP (10G)')); made_connection_this_pair = True
                        else: uhd_dev.return_port(port_uhd)
            if made_connection_this_pair: connection_made_in_full_pass = True
    # print(f"DIAG (Fill): 填充结束，找到 {len(new_connections)} 条新连接。") # 减少打印
    return new_connections

# --- 环形填充函数 ---
def fill_ring_connections(devices):
    """使用剩余端口，尝试添加更多环形连接段"""
    if len(devices) < 2: return []
    ...
    temp_devices = [copy.deepcopy(dev) for dev in devices] # 使用当前状态的副本
    device_map = {dev.id: dev for dev in temp_devices}
    sorted_dev_ids = sorted([d.id for d in devices]) # 按 ID 排序
    num_devices = len(sorted_dev_ids)

    print(f"DIAG (Fill Ring): 开始填充，设备顺序: {[next(d.name for d in devices if d.id == i) for i in sorted_dev_ids]}")
    ...
    while connection_made_in_full_pass:
        ...
        for i in range(num_devices):
            ...
            dev2_id = sorted_dev_ids[(i + 1) % num_devices] # N -> 0

            dev1_copy = device_map[dev1_id]
            dev2_copy = device_map[dev2_id]
            original_dev1 = next(d for d in devices if d.id == dev1_id)
            original_dev2 = next(d for d in devices if d.id == dev2_id)

            # 尝试在此段添加一个连接
            port1, port2, conn_type = _find_best_single_link(dev1_copy, dev2_copy) # 这个函数会消耗副本端口

            if port1 and port2:
                new_connections.append((original_dev1, port1, original_dev2, port2, conn_type))
                connection_made_in_full_pass = True # 本轮有进展
                # print(f"DIAG (Fill Ring): 添加新连接段 {original_dev1.name}[{port1}] <-> {original_dev2.name}[{port2}]") # 减少打印

    print(f"DIAG (Fill Ring): 填充结束，找到 {len(new_connections)} 条新连接段。")
    return new_connections
# --- 结束环形填充函数 ---


# --- Matplotlib Canvas Widget (保持不变) ---
class MplCanvas(FigureCanvas):
    """用于嵌入 Matplotlib 图形的 Qt 组件"""
    ...
    def plot_topology(self, devices, connections, fixed_pos=None): # 添加 fixed_pos 参数
        """绘制拓扑图"""
        self.axes.cla()
        if not devices:
            self.axes.text(0.5, 0.5, '无设备数据', horizontalalignment='center', verticalalignment='center')
            self.draw()
            return None, None # 返回 None for fig and pos

        # --- 统一字体设置逻辑 ---
        chinese_font_name = find_chinese_font()
        font_prop = None
        current_font_family = 'sans-serif' # 默认回退
        if chinese_font_name:
            try:
                ...
                print(f"警告: 设置 Matplotlib 字体 '{chinese_font_name}' 失败: {e}")
        ...
        for dev in devices:
            ...
            if dev.type == 'MicroN UHD': node_colors.append('skyblue')
            elif dev.type == 'HorizoN': node_colors.append('lightcoral')
            elif dev.type == 'MicroN': node_colors.append('lightgreen')
        ...
        if connections:
            for conn in connections:
                ...
                if dev1.id in node_ids and dev2.id in node_ids:
                    edge_key = tuple(sorted((dev1.id, dev2.id)))
                    ...
                    if base_conn_type not in edge_counts[edge_key]:
                        edge_counts[edge_key][base_conn_type] = {'count': 0, 'details': conn_type}
        ...
        pos = None
        if fixed_pos:
            # 检查 fixed_pos 是否包含所有当前节点，如果不包含则重新计算
            current_node_ids = set(G.nodes())
            stored_node_ids = set(fixed_pos.keys())
            if current_node_ids == stored_node_ids: # 确保节点完全一致
                 pos = fixed_pos
                 # print("DIAG (Plot): 使用已存储的节点位置。") # 减少打印
            else:
        ...
        if not pos:
            ...
            print("DIAG (Plot): 重新计算节点位置。")
        # --- 结束修改点 ---


        nx.draw_networkx_nodes(G, pos, node_color=node_colors, node_size=3500, ax=self.axes, alpha=0.8)
        nx.draw_networkx_labels(G, pos, labels=node_labels, font_size=9, ax=self.axes, font_family=current_font_family)

        if connections and G.edges(): # 确保图中有边再绘制
            ...
            for u, v in unique_edges:
                ...
                edge_color_map.append(color_found)
            nx.draw_networkx_edges(G, pos, edgelist=unique_edges, edge_color=edge_color_map, width=1.5, ax=self.axes, alpha=0.7, arrows=False)
            nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_size=7, ax=self.axes, label_pos=0.5, rotate=False, font_family=current_font_family)

        self.axes.set_title("网络连接拓扑图", fontproperties=font_prop)
        self.axes.axis('off')
        legend_elements = [
            ...
            plt.Line2D([0], [0], color='blue', lw=2, label='LC-LC (100G)'),
            plt.Line2D([0], [0], color='red', lw=2, label='MPO-MPO (25G)'),
            plt.Line2D([0], [0], color='orange', lw=2, label='MPO-SFP (10G)'),
            plt.Line2D([0], [0], color='purple', lw=2, label='SFP-SFP (10G)')
        ]
        ...
        return self.fig, pos # 返回 fig 和 pos

# --- 主窗口 ---
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MediorNet TDM Mesh 连接计算器 (PySide6)")
        self.setGeometry(100, 100, 1100, 800)

        self.devices = []
        self.connections_result = []
        self.fig = None
        self.node_positions = None # <--- 新增：存储节点位置
        ...
        main_splitter = QSplitter(Qt.Horizontal); main_layout.addWidget(main_splitter) # 使用 Splitter
        ...
        add_title = QLabel("<b>添加新设备</b>"); add_title.setFont(QFont(self.chinese_font.family(), 11)); add_group_layout.addWidget(add_title, 0, 0, 1, 2, alignment=Qt.AlignCenter)
        ...
        self.add_button = QPushButton("添加设备"); self.add_button.setFont(self.chinese_font); self.add_button.clicked.connect(self.add_device); add_group_layout.addWidget(self.add_button, 6, 0, 1, 2, alignment=Qt.AlignCenter)
        left_layout.addWidget(add_group)
        list_group = QFrame(); list_group_layout = QVBoxLayout(list_group); list_group_layout.setContentsMargins(10, 15, 10, 10)
        ...
        self.device_tablewidget = QTableWidget(); self.device_tablewidget.setFont(self.chinese_font); self.device_tablewidget.setColumnCount(6); self.device_tablewidget.setHorizontalHeaderLabels(["名称", "类型", "MPO", "LC", "SFP+", "连接数(估)"]); self.device_tablewidget.setSelectionBehavior(QAbstractItemView.SelectRows); self.device_tablewidget.setSelectionMode(QAbstractItemView.SingleSelection); self.device_tablewidget.setEditTriggers(QAbstractItemView.NoEditTriggers); self.device_tablewidget.setSortingEnabled(True)
        header = self.device_tablewidget.horizontalHeader(); header.setSectionResizeMode(QHeaderView.Interactive); header.setSectionResizeMode(0, QHeaderView.Stretch)
        ...
        file_group = QFrame(); file_group_layout = QGridLayout(file_group); file_group_layout.setContentsMargins(10, 15, 10, 10); file_title = QLabel("<b>文件操作</b>"); file_title.setFont(QFont(self.chinese_font.family(), 11)); file_group_layout.addWidget(file_title, 0, 0, 1, 2, alignment=Qt.AlignCenter)
        ...
        right_layout = QVBoxLayout(right_panel); main_splitter.addWidget(right_panel) # 添加到 Splitter

        # --- 计算控制区域 ---
        calculate_control_frame = QFrame()
        calculate_control_layout = QHBoxLayout(calculate_control_frame)
        calculate_control_layout.setContentsMargins(0, 5, 0, 5)
        ...
        calculate_control_layout.addStretch(); right_layout.addWidget(calculate_control_frame) # 使用 Frame
        ...
        self.edit_tab = QWidget(); edit_main_layout = QVBoxLayout(self.edit_tab); add_manual_group = QFrame(); add_manual_group.setFrameShape(QFrame.StyledPanel); add_manual_layout = QGridLayout(add_manual_group); add_manual_layout.setContentsMargins(10,10,10,10); add_manual_layout.addWidget(QLabel("<b>添加手动连接</b>", font=self.chinese_font), 0, 0, 1, 4, alignment=Qt.AlignCenter)
        ...
        self.add_manual_button = QPushButton("添加连接"); self.add_manual_button.setFont(self.chinese_font); self.add_manual_button.clicked.connect(self.add_manual_connection); add_manual_layout.addWidget(self.add_manual_button, 3, 0, 1, 4, alignment=Qt.AlignCenter)
    ...
    def _add_device_to_table(self, device):
        ...
        name_item = QTableWidgetItem(device.name); name_item.setData(Qt.UserRole, device.id)
        type_item = QTableWidgetItem(device.type)
        mpo_item = QTableWidgetItem(str(device.mpo_total)); mpo_item.setTextAlignment(Qt.AlignCenter)
        lc_item = QTableWidgetItem(str(device.lc_total)); lc_item.setTextAlignment(Qt.AlignCenter)
        sfp_item = QTableWidgetItem(str(device.sfp_total)); sfp_item.setTextAlignment(Qt.AlignCenter)
        conn_item = QTableWidgetItem(f"{device.connections:.2f}"); conn_item.setTextAlignment(Qt.AlignCenter)
    ...
    def add_device(self):
        ...
        self.node_positions = None # 添加设备后重置布局
    ...
    def remove_device(self):
        ...
        ids_to_remove = {self.device_tablewidget.item(row_index, 0).data(Qt.UserRole) for row_index in selected_rows if self.device_tablewidget.item(row_index, 0)}
        ...
        self._update_device_combos(); self.clear_results()
        self.node_positions = None # 移除设备后重置布局

    @Slot()
    def clear_all_devices(self):
        """清空所有设备"""
        if QMessageBox.question(self, "确认", "确定要清空所有设备吗？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) == QMessageBox.Yes:
            self.devices = []; self.device_tablewidget.setRowCount(0); self.device_id_counter = 0;
            self._update_device_combos(); self.clear_results()
            self.node_positions = None # 清空后重置布局

    @Slot()
    def clear_results(self):
        """清空计算结果和图形"""
        self.connections_result = []; self.fig = None; self.node_positions = None # 清空结果时重置布局
    ...
    def show_device_details_from_table(self, item):
         ...
         dev_id = name_item.data(Qt.UserRole)
    ...
    def calculate_and_display(self):
        ...
        self.node_positions = None # <--- 重置节点位置，强制重新布局

        mode = self.topology_mode_combo.currentText()
        calculated_connections, final_device_state_map = [], {}

        if mode == "Mesh": calculated_connections, final_device_state_map = calculate_mesh_connections(self.devices)
        elif mode == "环形": calculated_connections, final_device_state_map = calculate_ring_connections(self.devices)
        ...
        # 重建 port_connections
        for dev in self.devices: dev.port_connections.clear()
        for conn in self.connections_result:
            dev1, port1, dev2, port2, _ = conn
            if port1 not in dev1.port_connections: dev1.port_connections[port1] = dev2.name
            if port2 not in dev2.port_connections: dev2.port_connections[port2] = dev1.name
    ...
    def _update_connection_views(self):
        ...
        if self.connections_result:
            for i, conn in enumerate(self.connections_result):
                ...
                item = QListWidgetItem(item_text); item.setData(Qt.UserRole, conn); self.manual_connection_list.addItem(item)
            self.remove_manual_button.setEnabled(True)
        else: self.remove_manual_button.setEnabled(False)

        # --- 修改点: 传递并存储节点位置 ---
        self.fig, calculated_pos = self.mpl_canvas.plot_topology(self.devices, self.connections_result, fixed_pos=self.node_positions)
        if calculated_pos is not None and self.node_positions is None: # 如果是新计算的位置，则存储
    ...
    def _update_device_table_connections(self):
        ...
        for row in range(self.device_tablewidget.rowCount()):
            ...
            if name_item:
                dev_id = name_item.data(Qt.UserRole)
                device = next((d for d in self.devices if d.id == dev_id), None)
                if device: conn_item = QTableWidgetItem(f"{device.connections:.2f}"); conn_item.setTextAlignment(Qt.AlignCenter); self.device_tablewidget.setItem(row, 5, conn_item)
    ...
    def remove_manual_connection(self):
        ...
        for item in selected_items: conn_data = item.data(Qt.UserRole); connections_to_remove_data.append(conn_data); items_to_remove_from_widget.append(item)
        for conn_data in connections_to_remove_data:
            dev1_orig, port1, dev2_orig, port2, conn_type = conn_data # 获取原始对象引用
            try:
                # 查找并移除连接 (基于对象 ID 和端口名)
                found_index = -1
                ...
                if found_index != -1:
                    ...
                    # 归还端口到原始设备对象
                    dev1_orig.return_port(port1)
                    dev2_orig.return_port(port2)
        ...
        if connections_removed_count > 0:
             self.node_positions = None # 手动移除后重置布局
             for item in items_to_remove_from_widget: row = self.manual_connection_list.row(item); self.manual_connection_list.takeItem(row)
             # 重新编号
             for i in range(self.manual_connection_list.count()):
                 item = self.manual_connection_list.item(i); conn_data = item.data(Qt.UserRole)
                 if conn_data and len(conn_data) == 5: dev1, port1, dev2, port2, conn_type = conn_data; item.setText(f"{i+1}. {dev1.name} [{port1}] <-> {dev2.name} [{port2}] ({conn_type})")
                 else: item.setText(f"{i+1}. [错误的数据]")
             self._update_connection_views(); self._update_device_table_connections()
             self._update_device_combos() # 更新手动添加的下拉框
             print(f"成功移除了 {connections_removed_count} 条连接。")
             # 更新填充按钮状态
             enable_fill = bool(self.connections_result) or any(dev.get_all_available_ports() for dev in self.devices)
             self.fill_mesh_button.setEnabled(enable_fill)
             self.fill_ring_button.setEnabled(enable_fill)
        else: print("没有连接被移除。")
    ...
    def add_manual_connection(self):
        ...
        port1 = self.edit_port1_combo.currentText()
        port2 = self.edit_port2_combo.currentText()

        # --- 基本验证 ---
        if dev1_id is None or dev2_id is None or port1 == "选择端口..." or port2 == "选择端口...":
        ...
        dev1 = next((d for d in self.devices if d.id == dev1_id), None)
        dev2 = next((d for d in self.devices if d.id == dev2_id), None)
        if not dev1 or not dev2:
            QMessageBox.critical(self, "内部错误", "找不到选定的设备对象。")
            return

        # --- 端口类型和兼容性验证 ---
        port1_type = "LC" if port1.startswith("LC") else ("SFP" if port1.startswith("SFP") else ("MPO" if port1.startswith("MPO") else "Unknown"))
        port2_type = "LC" if port2.startswith("LC") else ("SFP" if port2.startswith("SFP") else ("MPO" if port2.startswith("MPO") else "Unknown"))
        conn_type_str = ""
        valid_connection = False
        dev1_ref, dev2_ref = dev1, dev2 # 保留原始引用，以防下面交换

        if port1_type == "LC" and port2_type == "LC" and dev1_ref.type in ['MicroN UHD', 'HorizoN'] and dev2_ref.type in ['MicroN UHD', 'HorizoN']:
            valid_connection = True; conn_type_str = "LC-LC (100G)"
        elif port1_type == "MPO" and port2_type == "MPO" and dev1_ref.type in ['MicroN UHD', 'HorizoN'] and dev2_ref.type in ['MicroN UHD', 'HorizoN']:
            valid_connection = True; conn_type_str = "MPO-MPO (25G)"
        elif port1_type == "SFP" and port2_type == "SFP" and dev1_ref.type == 'MicroN' and dev2_ref.type == 'MicroN':
             valid_connection = True; conn_type_str = "SFP-SFP (10G)"
        elif port1_type == "MPO" and port2_type == "SFP" and dev1_ref.type in ['MicroN UHD', 'HorizoN'] and dev2_ref.type == 'MicroN':
             ...
             # 交换顺序以便记录连接和执行 use_specific_port
             dev1, dev2 = dev2_ref, dev1_ref
             port1, port2 = port2, port1
             valid_connection = True; conn_type_str = "MPO-SFP (10G)" # 规范化类型字符串

        if not valid_connection:
            QMessageBox.warning(self, "连接无效", f"设备类型 '{dev1_ref.type}' 的端口 '{port1_type}' 与 设备类型 '{dev2_ref.type}' 的端口 '{port2_type}' 之间不允许连接。")
            return

        # --- 尝试使用端口并添加连接 ---
        # 注意：现在 dev1, port1, dev2, port2 可能已经被交换过了 (对于 SFP-MPO 情况)
        if dev1.use_specific_port(port1, dev2.name):
            if dev2.use_specific_port(port2, dev1.name):
                # 成功使用两个端口
                self.node_positions = None # 手动添加连接后重置布局
                # 添加连接时，使用原始的 dev1_ref, dev2_ref 以保持用户选择的顺序感（尽管内部可能交换了）
                self.connections_result.append((dev1_ref, port1, dev2_ref, port2, conn_type_str))
                self._update_connection_views()
                self._update_device_table_connections()
                self._populate_edit_port_combos(self.edit_dev1_combo, self.edit_port1_combo) # 更新端口下拉框
                self._populate_edit_port_combos(self.edit_dev2_combo, self.edit_port2_combo)
                print(f"成功添加手动连接: {dev1_ref.name}[{port1}] <-> {dev2_ref.name}[{port2}]")
                # 手动添加后启用填充按钮
                enable_fill = bool(self.connections_result) or any(dev.get_all_available_ports() for dev in self.devices)
                self.fill_mesh_button.setEnabled(enable_fill)
                self.fill_ring_button.setEnabled(enable_fill)
                ...
                # 第二个端口使用失败，归还第一个端口
                dev1.return_port(port1)
                QMessageBox.critical(self, "错误", f"端口 '{port2}' 在设备 '{dev2.name}' 上不可用或已被占用。")
                # 需要刷新第一个下拉框，因为端口已归还
                self._populate_edit_port_combos(self.edit_dev1_combo if dev1.id == dev1_id else self.edit_dev2_combo,
                                                self.edit_port1_combo if dev1.id == dev1_id else self.edit_port2_combo)
        else:
            QMessageBox.critical(self, "错误", f"端口 '{port1}' 在设备 '{dev1.name}' 上不可用或已被占用。")
    ...
    def fill_remaining_mesh(self):
        ...
        print("开始填充剩余连接 (Mesh)...")
        new_connections = fill_mesh_connections(self.devices) # 传递原始设备列表

        if new_connections:
            print(f"DIAG (Fill): 找到 {len(new_connections)} 条新连接可以添加。")
            self.connections_result.extend(new_connections)
            print("DIAG (Fill): 更新原始设备状态...")
            for conn in new_connections:
                dev1, port1, dev2, port2, _ = conn
                if not dev1.use_specific_port(port1, dev2.name): print(f"严重警告: Mesh 填充时未能使用端口 {dev1.name}[{port1}]")
                if not dev2.use_specific_port(port2, dev1.name): print(f"严重警告: Mesh 填充时未能使用端口 {dev2.name}[{port2}]")
            print("DIAG (Fill): 更新视图...")
            # --- 修改点: 填充后不重置布局 ---
            # self.node_positions = None # 移除这行
            ...
            print("DIAG (Fill): 未找到可填充的新 Mesh 连接。")
        # 填充后禁用填充按钮
        self.fill_mesh_button.setEnabled(False)
        self.fill_ring_button.setEnabled(False)
    ...
    def fill_remaining_ring(self):
        ...
        print("开始填充剩余连接 (环形)...")
        new_connections = fill_ring_connections(self.devices) # 传递原始设备列表

        if new_connections:
            print(f"DIAG (Fill Ring): 找到 {len(new_connections)} 条新连接可以添加。")
            self.connections_result.extend(new_connections)
            print("DIAG (Fill Ring): 更新原始设备状态...")
            for conn in new_connections:
                dev1, port1, dev2, port2, _ = conn
                if not dev1.use_specific_port(port1, dev2.name): print(f"严重警告: 环形填充时未能使用端口 {dev1.name}[{port1}]")
                if not dev2.use_specific_port(port2, dev1.name): print(f"严重警告: 环形填充时未能使用端口 {dev2.name}[{port2}]")
            print("DIAG (Fill Ring): 更新视图...")
            # --- 修改点: 填充后不重置布局 ---
            # self.node_positions = None # 移除这行
            self._update_connection_views(); self._update_device_table_connections(); self._update_device_combos()
            ...
            print("DIAG (Fill Ring): 未找到可填充的新环形连接段。")
        # 填充后禁用填充按钮
        self.fill_mesh_button.setEnabled(False)
        self.fill_ring_button.setEnabled(False)

    # --- (其他方法保持不变: save_config, load_config, export_*, filter_device_table) ---
    @Slot()
    ...
    def load_config(self):
        """从 JSON 文件加载设备列表"""
        if self.devices and QMessageBox.question(self, "确认", "加载配置将覆盖当前设备列表，确定吗？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No) == QMessageBox.No: return
        ...
        try:
            ...
            self.node_positions = None # 加载后重置布局
...
def find_chinese_font():
    """查找系统中的可用中文字体"""
    font_names = ['SimHei', 'Microsoft YaHei', 'PingFang SC', 'Heiti SC', 'STHeiti', 'Noto Sans CJK SC', 'WenQuanYi Micro Hei', 'sans-serif'] # 添加更多 macOS 字体
    ...
    # 优先返回 PingFang SC (macOS), Microsoft YaHei (Windows), SimHei (通用)
